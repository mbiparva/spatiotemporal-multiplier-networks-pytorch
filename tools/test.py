...
if __name__ == '__main__':
    ...
    cfg.CUS_PT_PATH = {  # setting the custom network snapshot to be loaded
        'image': os.path.join(cfg.EXPERIMENT_DIR,
                              'snapshot_{}_{}'.format(cfg.DATASET_NAME, cfg.NET_ARCH),
                              args.pt_id,
                              'spt_{:03}.pt'.format(args.pt_epoch)),
        'flow': os.path.join(cfg.EXPERIMENT_DIR,
                             'snapshot_{}_{}'.format(cfg.DATASET_NAME, cfg.NET_ARCH),
                             args.pt_id,
                             'tmp_{:03}.pt'.format(args.pt_epoch))
    }
