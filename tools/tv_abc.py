...
class TVBase(ABC):
    ...
    def generate_gt(self, annotation):
        a = torch.zeros(len(annotation), dtype=torch.long, device=self.device)
        for i, k in enumerate(annotation):
            a[i] = k['label']
        return a
    ...
    @abstractmethod
    def batch_main(self, net, image, flow, annotation):
        pass

    def batch_loop(self, net, epoch, started_time):
        batch_time = AverageMeter()
        end = time.time()
        for i, (image, flow, annotation) in enumerate(self.data_container.dataloader):

            image = image.to(self.device)
            flow = flow.to(self.device)

            results = self.batch_main(net, image, flow, annotation)
